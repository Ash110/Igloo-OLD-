...
import {Button, TextField, CircularProgress, Checkbox, ListItem, ListItemAvatar, ListItemText, List, IconButton, FormGroup,FormControlLabel,Switch } from '@material-ui/core'
...
class PostUploader extends Component {
    ...
    rotateImageSrc = () => {
        ...
        uploadImage = async () => {
            if(this.state.selectedGroups.length > 0){
                ...
                const disableComments = this.state.disableComments;
                axios.post('/api/posts/newPost', { image, isText,isTemp : false, caption, groups: this.state.selectedGroups, disableComments }, config)
                    .then(response => {
        ...
        render() {
            ...
            return (
                <div id="postUploader">
                    <h3 style={{ width: "100%", textAlign: "center" }}>Post an image!</h3>
                    <br /><br /><br />
                    <div>
                    ...
                    {src && (
                        <React.Fragment>
                            ...
                            <FormGroup row>
                                <FormControlLabel
                                    control={<Switch checked={this.state.disableComments} onChange={(e) => { this.setState({ disableComments: e.target.checked }) }} name="timed" />}
                        ...
                        <br/><br/>
                            {this.state.submittingStage === 1 ? <small>We store posts in full high quality, without compression, so it might take a while to upload. Hang in there!</small> : ""}
                        </React.Fragment>
                    )}
                </div>
            );
    ...
    export default connect(mapStateToProps, null)(PostUploader);
