...
class Post extends Component {
    ...
    componentDidMount() {
        ...
        axios.post('/api/pages/getPostDetails', { postId: this.props.id }, config)
            .then(async (res) => {
                ...
                if (!isText) {
                    var image = "data:image/png;base64," + Buffer.from(await getPagePostImage(this.props.id), 'binary').toString('base64')
                } else {
                    image = null;
                }
    ...
    likePost = () => {
        ...
        axios.post('/api/pages/likePost', { postId: this.props.id }, config)
            .then((res) => {
    ...
    unlikePost = () => {
        ...
        axios.post('/api/pages/unlikePost', { postId: this.props.id }, config)
            .then((res) => {
    ...
    submitComment = () => {
        ...
        if (comment) {
            axios.post('/api/comments/postComment', { postId: this.props.id, comment, type:"pagePost" }, config)
                .then((res) => {
    ...
    deletePost = () => {
        ...
        if (x) {
            ...
            axios.post('/api/pages/deletePost', { postId: this.props.id }, config)
                .then((res) => {
    ...
    render() {
        ...
        const postLink = "/pagepost/" + this.props.id;
            ...
            return (
                <div id="pagePostContainer" className="animated pulse faster">
                    <Dialog onClose={this.handleClose} aria-labelledby="simple-dialog-title" open={this.state.showLikes}>
                        ...
                        <ViewLikes type="pagepost" id={this.props.id} />
                    </Dialog>
